...
$(document).ready(function () {
	var width = screen.width;//$('#wrapper')[0].clientWidth;
	var height = screen.height;//$('#wrapper')[0].clientHeight;
	var w = width / size;
	var h = height / size;
	// console.log("Width: ");
	// console.log(width);
    ...
    for (var j = 0; j < rows; j++) {
	    for (var i = 0; i < columns; i++) {
	    	sq = $square.clone();
	    	sq[0].style.left = i*size;
	    	sq[0].style.top = j*size;
	    	sq[0].id = i.toString()+ j.toString();
	    	sq[0].test = j*size;
